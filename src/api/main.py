...
# THIS IS THE CODE FOR TFIDF NLP MODEL WHICH WAS THEN UPGRADED TO OPENAI EMBEDDINGS MODEL
...
# # ❌ Removed spaCy completely
# app = FastAPI(title="ATS Resume Ranking Portal")
# templates = Jinja2Templates(directory="src/api/templates")

# # ✔ New lightweight clean_text (Render compatible)
# def clean_text(text):
#     text = text.lower()
#     text = re.sub(f"[{re.escape(string.punctuation)}]", " ", text)
#     text = re.sub(r"\d+", " ", text)
